...
export default function TopBar() {
  ...
  const humanRoomName = useMemo(() => {
    return roominfo_after_enter && roominfo_after_enter.room_name
      ? decodeURI(roominfo_after_enter.room_name)
      : undefined;
  ...
  return (
    <div className="navbar flex fixed h-16 bg-base-100 bg-transparent z-10 backdrop-blur-lg">
      ...
      <div className="flex-none z-10">
        ...
        {!isMobile && (
          <label htmlFor="topBarModal" className="btn btn-ghost normal-case  text-center text-xl">
            <svg
              className="icon  text-primary-focus"
              viewBox="0 0 1024 1024"
              version="1.1"
        ...
        <div className="modal h-screen">
          <div className="modal-box max-w-[1000px] max-h-[100%] bg-neutral-content">
            <label
              htmlFor="topBarModal"
            ...
            <LocalRecorderComponent recordingChange={recordingChange}></LocalRecorderComponent>
          </div>
        </div>
        ...
        {isjoin && humanRoomName && (
          <div>
            ...
            <label htmlFor="infoModal" className="modal h-screen">
              <label className="modal-box relative bg-primary" htmlFor="">
                <div>
                  <RoomInfo roomName={humanRoomName} join={isjoin} />
                  <div className=" divider"></div>
                  <span className=" text-lg">
                    {t('room.isAdmin') + ': ' + (mcurState.isAdmin ? 'yes' : 'no')}
                  </span>
                  <br />
                  <span className=" text-lg">
                    {t('room.passwd') +
                      ': ' +
                      (roominfo_after_enter.passwd ? roominfo_after_enter.passwd : 'no passwd')}
                  </span>
                  <br />
                </div>
              </label>
      ...
      {/* toast */}
      {showToast && <MyInfoToast>{TostMsg}</MyInfoToast>}
    </div>
  );
}
